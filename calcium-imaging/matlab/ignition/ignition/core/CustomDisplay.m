classdef (HandleCompatible) CustomDisplay < matlab.mixin.CustomDisplay
	...
	methods (Access = protected)
		function propGroups = getPropertyGroups(obj)
			...
			propGroupCache = ignition.util.persistentMapVariable(propGroupCache);
			className = class(obj);
			cacheKey = strrep(className,'.','_');
			
			% USE LIST OF PUBLIC PROPERTIES (FAST/BUILTIN) TO INDICATE CLASS-DEFINITION
			propListFailFast = ignition.util.persistentMapVariable(propListFailFast);
			...
			propListFailFast(cacheKey) = publicPropHashVec;
			ignition.core.CustomDisplay.getSetStaticCache(propGroupCache);
	...
	methods (Access = public, Hidden)
		function [propGroups, varargout] = getCustomPropertyGroups(obj)
			...
			if (nargout > 1)
				varargout{1} = ignition.core.CustomDisplay.getSetStaticCache();
			end
