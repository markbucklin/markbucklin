function [config, cache] = initializeTiffFileStream(config)
...
try
	...
	config.tagIDs = readableTagIDs;
	config.firstFrameTag = readableTiffTags;
	...
	% ----------------------------------------------
	% INITIALIZE DYNAMIC VARIABLES IN TASK-CACHE
	% ----------------------------------------------
	cache.NumFramesPerRead = 8;
	cache.FirstFrameIdx = 1;
	cache.LastFrameIdx = numFramesTotal;
	cache.NextFrameIdx = 0; % 1:taskCache.NumFramesPerRead;
	cache.PriorFrameIdx = 0;
	cache.StreamFinishedFlag = false;
	
	
	% 	taskCache.CurrentTiffObj = firstTiffObj;
	% 	taskCache.NextFileIdx = configCache.lookupFileIdx( taskCache.NextFrameIdx);
	% 	taskCache.PriorFileIdx = 0;
	...
	assignin('base','configCache',config);
	msg = getReport(me);
	disp(msg);
	rethrow(me)
end
