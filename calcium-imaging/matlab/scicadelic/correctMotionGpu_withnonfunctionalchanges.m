function [F, varargout] = correctMotionGpu(movingInput, fixedLocalInput, fixedGlobalInput, subPix, useLog, antiEdgeWin, peakInterpBicubic, resampleBicubic)
...
% ##################################################
	...
	function fk = bicubicImageResampleKernel( rowC, colC, t, dy, dx)
		...
		% COMPUTE OVER COLUMNS TO GET VALUES AT X
		fL = catmullromcolkern(dxLL);
		f0 = catmullromcolkern(dxL);
		fR = catmullromcolkern(dxR);
		f3 = catmullromcolkern(dxRR);
		
		fk = single(0) ...
			+ fL * catmullkern(sx-dxLL) ...
			+ f0 * catmullkern(sx-dxL) ...
			+ fR * catmullkern(sx-dxR) ...
			+ f3 * catmullkern(sx-dxRR);
		...
		function f = resamplePixel(uyk,uxk)
			...
			if (frow~=yk) || (fcol~=xk)
				f = single(fixedGlobalInput(max(1,min(numRows, y0)), max(1,min(numCols,x0))));
		...
		function fcol = catmullromcolkern(dxk)
			fcol = single(0) ...
				+ resamplePixel(dyUU, dxk) * catmullkern(sy-dyUU) ...
				+ resamplePixel(dyU , dxk) * catmullkern(sy-dyU) ...
				+ resamplePixel(dyD, dxk) * catmullkern(sy-dyD) ...
				+ resamplePixel(dyDD, dxk) * catmullkern(sy-dyDD);
