function dFs = sampledSurroundBestDiff_tryspmd(F, ds, Fthresh)
...
spmd(8)
	...
	FdiffComposite = int16(F)-int16(Fs);
	gtMaskPos = bsxfun(@gt, FdiffComposite , Fthresh);
	gtMaskNeg = bsxfun(@lt, FdiffComposite , -Fthresh);
...
maskPos = countPosNeg >  6;
maskNeg = countPosNeg < -6;
