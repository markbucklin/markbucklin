function [S, varargout] = updatePeakBorderDistRunGpuKernel(P, S0, rowSubs, colSubs, frameSubs)
...
% ##################################################
	function [sPxNew, isPxPeak, isPxBorder] = peakBorderDistKernel(pPx, sPx, rowC, colC, n)
		...
		if (sPx == 0)
			...
			if isPxBorder
				...
				if (pPx > 0) % or... (sPx > 0)
					...
					sBlockMin = min(min(min(min(min(min(min(sUL,sUC),sUR),sDL),sDC),sDR),sCL),sCR);
					...
					isPxPeak = (pPx > pSurrMax);
					
				else
					% FIND MAX DIST(K-1) FROM NEIGHBOR PIXELS: (DARK/VESSEL-LAYER -> DIST FROM CELL)
					sBlockMax = max(max(max(max(max(max(max(sUL,sUC),sUR),sDL),sDC),sDR),sCL),sCR);
