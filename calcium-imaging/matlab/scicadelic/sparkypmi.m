...
proc.tgtf = scicadelic.TemporalFilter('MinTimeConstantNumFrames',8);
...
BW = false(numRows,numCols,numPmiChunks);
BWL = false(numRows,numCols,numPmiChunks);
P = [];
Qmin = [];
P0 = [];
bw = [];
...
while ~isempty(idx) && (idx(end) < N)
	...
	if idx(1) > idx0
		...
		BW(:,:,dm) = gather(bw);
		bwl = reshape(  bwmorph( reshape(bw,numRows,[],1),'shrink', inf)   ,numRows,numCols,[]);
		BWL(:,:,dm) = gather(bwl);
		
		% 		BWL(:,:,dm) = gather(bwmorph( bwmorph( bwmorph( bw,'clean'), 'majority'), 'shrink', inf)); % bwmorph(bw,'majority',1)
		
		% 		[Iy,Ix,Isym] = mapInformationTopologyRunGpuKernel(PMI);
		% 		S0 = pushSeedSupportRunGpuKernel(Iy,Ix,Isym,25);
		% 		S0rgb = uint8(cat(3, S0, 25*all(S0,3))).*10;
		% 		S0cpu(:,:,:,dm) = gather(S0rgb);
		% 		newmax(:,:,dm) = gather(proc.sc.Max - oldmax);
		

		
		% 		Iycpu(:,:,1,dm) = gather(int8(128.*mean(Iy,3)));
		% 		Ixcpu(:,:,1,dm) = gather(int8(128.*mean(Ix,3)));
		% 		Isymcpu(:,:,1,dm) = gather(mean(Isym,3));
		
		% pmi(:,:,:,dm) = gather( int8( 127.*PMI) );
