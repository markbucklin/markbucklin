...
while idx(end) < N
	...
	loadTime = toc(t);
	fprintf('Load: %3.3gms\t',1000*loadTime/framesPerStep); t=tic;
	...
	processTime = toc(t);
	fprintf('PreProc: %3.3gms\t',1000*processTime/framesPerStep);t=tic;
	
	
	% NEW: NEIGHBORHOODSAMPLING
	if isempty(L)
		[L, P, LFS, Lsize, lockedLabel, Lcnt] = localLabelSampler(F);
	else
		if isempty(P)
			[L, P, LFS, Lsize, lockedLabel, Lcnt] = localLabelSampler(F,L);
		else
			if isempty(lockedLabel)
				[L, P, LFS, Lsize, lockedLabel, Lcnt] = localLabelSampler(F,L,P);
			else
				if isempty(Lcnt)
					[L, P, LFS, Lsize, lockedLabel, Lcnt] = localLabelSampler(F,L,P, lockedLabel);
				else
					[L, P, LFS, Lsize, lockedLabel, Lcnt] = localLabelSampler(F,L,P, lockedLabel, Lcnt);
				end
			end
		end
	end
	processTime = toc(t);
	fprintf('Label: %3.3gms\t',1000*processTime/framesPerStep);t=tic;
	
	
	% STATISTIC COLLECTION
	step(SC, F)
	processTime = toc(t);
	fprintf('Stats: %3.3gms\t',1000*processTime/framesPerStep);t=tic;
...
% 	lCell{m} = oncpu(nonzeros(L));		
	...
	processTime = toc(t);
	fprintf('GPU2CPU: %3.3gms\n',1000*processTime/framesPerStep);
