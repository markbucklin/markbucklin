function [Fcell, scicadelicPathCleaner, TL] = runScicadelicVidGen_susie(TL)
...
proc.tf = scicadelic.TemporalFilter('MinTimeConstantNumFrames',4);
...
while ~isempty(idx) && (idx(end) < stopIdx)
	...
	try
		...
		fRed = dstat.M4;
		fRed = step(MF, fRed);
		[fRed, gmcRstat] = gemanMcClureNormalizationRunGpuKernel(fRed, gmcRstat);
		fGreen = F;
		fGreen = step(INg, fGreen);
		fBlue = gdstat.M3;
		fBlue = step(MF, fBlue);
		[fBlue, gmcBstat] = gemanMcClureNormalizationRunGpuKernel(fBlue, gmcBstat); % if < 512 use fB/max(fB)??
		%fBlue = fBlue .* 1.5; %new
		...
		fRed = max( fRed-fBlue, 0);%nextrun % or fBlue = fBlue .* (1-fRed);
		rbc = single(0.2);
