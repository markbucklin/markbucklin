function scicadelicPathCleaner = runScicadelicVidGen_bw(TL)
...
mp4FileName = [videoFileNameRoot, '.mp4'];
...
writerObj.Quality = 98; %new (was 95)
...
while ~isempty(idx) && (idx(end) < stopIdx)
	...
	try
		...
		fcpu = gather(F);		
		binaryFileName = writeBinaryData(fcpu, videoFileNameRoot, false);
		
% 		fcpu = double(1 + 255.*fcpu);
% 		cmap = gray(256);
% 		for kf=1:size(fcpu,3)
% 			writeVideo(writerObj, im2frame(fcpu(:,:,kf), cmap));
% 		end
