...
hVideoOut.Position(3:4) = [1024 1024];

%%
% Here we initialize some variables used in the processing loop.
pos.template_orig = [200 200]; % [x y] upper left corner
...
kFrame = 1;
while kFrame<numel(vid)
    input = double(vid(kFrame).cdata);
    ...
    Stabilized(BorderRows, :) = 0;
	 ...
	 vidstab(kFrame).cdata = Stabilized;
	 vidstab(kFrame).MotionVector = MotionVector;
	 vidstab(kFrame).idx = Idx;
	 fprintf('frame %i\n',kFrame)
