function S = seedMapRegisterDetectedRegionsRunGpuKernel(Sdet, Sreg, seedMap)
...
%		RTSCI.PIXELLABEL>APPLYLABELEDREGIONSTATISTICUPDATE. It follows a call from another
%		RTSCI.PIXELLABEL method to the function FINDCONNECTEDREGIONPROPSRUNGPUKERNEL, which
...
%
% See Also:
%		rtsci.PixelLabel, FINDCONNECTEDREGIONPROPSRUNGPUKERNEL, rtsci.StatisticCollector,
...
if isempty(seedMap)
	...
	% 	[seedRow,seedCol,seedRegIdx] = find(seedMap);
	% 	seedMap(seedRow,seedCol) = seedRegIdx;
...
lutSize = length(Sreg.Area);

% GET REGISTERED REGION SEED ROW/COL SUBSCRIPTS
seedRowSubs = Sreg.RegionSeedIdx(regIdxSubs,1);
seedColSubs = Sreg.RegionSeedIdx(regIdxSubs,2);
...
% ----------------------------------------------------
[Q, P, regIdx, detIdx, bbCornerX, bbCornerY, bbWidth, bbHeight, Cx, Cy, roiArea, roiFrameIdx] = ...
...
REGIDXLUT(detIdx(Q)+1) = uint16(regIdx(Q));

% RE-MAP LABEL MATRIX VALUES TO INSTEAD MAP INTO 'REGISTERED REGIONS' INDICES
S.LabelMatrix = arrayfun(@lookup, IDXMAPdet);
...
% BOUNDING-BOX
S.BoundingBox = [bbCornerX(Q), bbCornerY(Q), bbWidth(Q), bbHeight(Q)];

% CENTROID
S.Centroid = [Cx(Q) Cy(Q)];

% FRAME-INDEX
S.FrameIdx = roiFrameIdx(Q);

% LABEL-INDEX
S.RegionSeedIdx = int32(round([Cy(Q) Cx(Q)]));
...
% ##################################################
	function [q,p, ridx,didx, bx,by,dx,dy,cx,cy,a,kidx] = seedDetectedRegionKernelFcn(seedrow,seedcol,ridx,k)
		...
		if (didx < 1) || isnan(didx) || isinf(didx)
			...
			cx = single(nan);
			cy = single(nan);
			% 			bbc = single(nan + 1i*nan);
			% 			bbw = single(nan + 1i*nan);
			% 			c = single(nan + 1i*nan);
			kidx = single(nan);
			return
