	...
	function [amean,bmean,cmean,dmean,pseed,ksince,n] = pixelPropUpdateKernelFcn(rowIdx,colIdx,cnt)
		...
		while k < numFrames
			...
			if (pxlabel >= 1)
				...
				bx = min( abs(x - B(pxlabel,1)), abs(B(pxlabel,1) + dx + 0.5 - x));
				bxn = 2*bx/dx;
				by = min( abs(y - B(pxlabel,2)), abs(B(pxlabel,2) + dy + 0.5 - y));
				byn = 2*by/dy;
				b = bxn + 1i*byn;
				...
				% SEED PROBABILITY
				% 				ps = max(1 - (abs(c)*abs(dmean)), 0) * bxn * byn;
				ps = max(1 - (abs(c)), 0) * bxn * byn; % sin or tanh or exp
