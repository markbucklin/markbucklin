classdef (CaseInsensitiveProperties = true) TemporalGradientStatisticCollector < rtsci.StatisticCollector
	% TEMPORALGRADIENTSTATISTICCOLLECTOR - Computes running statistics after taking temporal derivative of input
	%
	%
	% Syntax:
	%			>> sc = rtsci.TemporalGradientStatisticCollector;
	%			>> step(sc, F);
	%
	% Description:
	...
	methods
		function obj = TemporalGradientStatisticCollector(varargin)
			...
			obj = obj@rtsci.StatisticCollector(varargin{:});
	...
	methods (Access = protected)
		...
		function setupImpl(obj, F)
			...
			setupImpl@rtsci.StatisticCollector(obj, Ft);
		...
		function varargout = stepImpl(obj, F)
			...
			if numArgs == 0
				stepImpl@rtsci.StatisticCollector(obj, Ft);
			elseif numArgs == 1
				availableArgs{1} = stepImpl@rtsci.StatisticCollector(obj, Ft);
			elseif numArgs == 2
				[availableArgs{1}, availableArgs{2}] = stepImpl@rtsci.StatisticCollector(obj, Ft);
			else
				[availableArgs{1}, availableArgs{2}, availableArgs{3}] = stepImpl@rtsci.StatisticCollector(obj, Ft);
			end
