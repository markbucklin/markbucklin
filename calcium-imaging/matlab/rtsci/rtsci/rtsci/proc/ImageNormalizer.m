classdef (CaseInsensitiveProperties = true) ImageNormalizer < rtsci.proc.PipedSystem
	% IMAGENORMALIZER - Normalizes input and scales to values between [0,1], [-sigma,sigma] (single) or [0,255] (uint8)
	%	
	%
	% Syntax:
	%			>> INr = rtsci.proc.ImageNormalizer;
	%			>> Fr = step(INr, F);
	%			>> INg = rtsci.proc.ImageNormalizer('NormalizationType','ExpNormComplement');
	%			>> Fg = step(INg, F);
	%			>> INb = rtsci.proc.ImageNormalizer('NormalizationType','Geman-McClure');
	%			>> Fb = step(INb, F);
	%
	% Description:
	...
	%
	% See Also: 
	%			BWMORPH GPUARRAY/BWMORPH UPDATESTATISTICSGPU, rtsci.proc.STATISTICCOLLECTOR
	...
	properties (Nontunable, SetAccess = ?rtsci.System)
		StatCollector		
	end
	...
	methods (Access = protected)
		...
		function setupImpl(obj, F)
			...
			% 			obj.StatCollector = rtsci.proc.StatisticCollector;
