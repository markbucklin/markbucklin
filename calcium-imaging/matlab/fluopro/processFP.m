function varargout = processFP(varargin)
% function [allVidFiles, R, info, uniqueFileName] = processFP(varargin)
...
try
   ...
   % ------------------------------------------------------------------------------------------
   %    try
   % 	  saveroifcn = @(sfroi,spath) save(fullfile(spath,'unreduced_single_frame_roi'),sfroi);
   % 	  saveJob = batch(saveroifcn(singleFrameRoi, fdir),0);
   %    catch me
   % 	  keyboard
   %    end
...
try
   ...
   procstart.procstep.roisegmented = createMask(singleFrameRoi(ismember([singleFrameRoi.Frames],representativeFrameIdx)));
catch me
   keyboard
end
end
...
N = size(data,3);
data = bsxfun( @minus, data+(intmax(class(data))/64), imclose(pre.fmin, strel('disk',2)));
...
maxPerimOverSqArea = 6.8; %  circle = 3.5449, square = 4 % Previousvalues: [6.5  ] 
