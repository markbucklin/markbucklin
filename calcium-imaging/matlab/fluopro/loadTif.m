...
for n = 1:nFiles
   tifFile(n).firstIdx = fileFrameIdx.first(n);
   tifFile(n).lastIdx = fileFrameIdx.last(n);
end
...
for n=1:numel(tifFile)
   ...
   for k = firstFrame:lastFrame
	  ...
	  if isfield(info(k).tiffTag, 'ImageDescription') %TODO
		 try
