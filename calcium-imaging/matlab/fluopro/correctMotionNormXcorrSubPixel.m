function xc = correctMotionNormXcorrSubPixel(datainput, fcnparam)
% not yet functional
% FLUOPRO
global FPOPTION
...
[nRows,nCols] = size(fixedFrame);
centerPoint = [ceil(nRows./2), ceil(nCols./2)]; %[rowidx, colidx]
fixedRowSubs = getSubsCenteredOn(centerPoint(1), fixedHeight);
fixedColSubs = getSubsCenteredOn(centerPoint(2), fixedWidth);
movingRowSubs = getSubsCenteredOn(centerPoint(1), movingHeight);
movingColSubs = getSubsCenteredOn(centerPoint(2), movingHeight);
getNewCenterPoint = @() [...
floor(movingHeight/2+1) + floor(rand(1)*(min(nRows)-movingHeight)) , ...
floor(movingWidth/2+1) + floor(rand(1)*(min(nCols)-movingWidth))];
...
for kFrame=1:N
	  ...
	  mFix = fixedRowSubs(:,1);	  
	  nFix = fixedColSubs(:,1);
	  mMov = movingRowSubs(:,1);
	  nMov = movingColSubs(:,1);
	  % LOAD AND FILTER FIXED & MOVING IMAGE-PATCHES
	  fixedPatch = gaussFilt(gpuArray(fixedFrame(mFix,nFix)));
	  movingPatch = gaussFilt(gpuArray(movingFrame(mMov,nMov)));
	  ...
	  cMax(kFrame,1) = gather(cmax);
	  yPeak(kFrame,1) = gather(ypeak);
	  xPeak(kFrame,1) = gather(xpeak);
%    end
   
   % RECORD MOTION-VECTORS & CENTER-POINTS
   Uy(kFrame,:) = yPeak(kFrame,:) - stationaryRow;
   Ux(kFrame,:) = xPeak(kFrame,:) - stationaryCol;
   cpY(kFrame,:) = centerPoint(:,1)';
   cpX(kFrame,:) = centerPoint(:,2)';
   ...
   utTol = std(uxy(:));
	  ...
	  if sum(abs(ut(1,:))) > utTol ...
			|| cframe(1,1) < ccTol
		  %*** make a function
		  % 		 kUglySub = kSubReg;
		 cframe(1,1) = NaN;
		 oldCenterPoint = centerPoint(1,:);
		 newCenterPoint = getNewCenterPoint();
		 cpShift = newCenterPoint - oldCenterPoint;%[rowidx colidx]
		 centerPoint(1,:) = newCenterPoint;
		 fixedRowSubs = addShift2Subs(fixedRowSubs, cpShift(1),1);
		 fixedColSubs = addShift2Subs(fixedColSubs, cpShift(2),1);
		 movingRowSubs = addShift2Subs(movingRowSubs, cpShift(1),1);
		 movingColSubs = addShift2Subs(movingColSubs, cpShift(2),1);
	  end
