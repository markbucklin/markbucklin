function code = TargetChaseExperimentCode
...
function vr = initializationCodeFun(vr)
    ...
    vr.vr.currentSquare = vr.SWtargetIndx;
end
...
if (vr.position(1)>(-vr.floor_width/4 - vr.target_size/2)...
    ...
    vr.vr.currentSquare = vr.NWtargetIndx;
    vr.worlds{vr.currentWorld}.surface.colors(1,vr.NWtargetIndx) = 0;
end
...
if (vr.worlds{vr.currentWorld}.surface.colors(1,vr.NEtargetIndx) == 0)...
    ...
    allSquares = {vr.NEtargetIndx,...
     ...
     if randSquare==vr.currentSquare;
         randSquare = allSquares{ceil(rand*6)};
     end
      if randSquare==vr.currentSquare;
         randSquare = allSquares{ceil(rand*6)};
...
%     if ~isempty(vr.currentSquare)
%         while randSquare == vr.currentSquare
%             randSquare = allSquares(ceil(rand*6));
%         end
%     end
    vr.worlds{vr.currentWorld}.surface.colors(1,randSquare) = 1;
    %              vr.numRewards = vr.numRewards - 1;
end
%     
    %reduction loops
