...
%full array of vertices involved with N target
...
%full array of vertices involved with S target
...
vr.target.N.on = true; %wich target actually gives reward
...
%% SET UP VARIABLES FOR "UNIDIRECTIONAL"
startLocation = -eval(vr.exper.variables.floor_height)/3;
...
%% MOUSE INSIDE TARGET AREA AND COLOR RESET/TELEPORT NORTH
try %N target
...
%% MOUSE INSIDE TARGET AREA AND COLOR RESET/TELEPORT SOUTH
try %S target
...
%% Trouble shooting
assignin('base','vr',vr)
%% velocity training within unidirectional (off)
persistent downCounter
 if isempty(downCounter)
     downCounter = 100;
 end
