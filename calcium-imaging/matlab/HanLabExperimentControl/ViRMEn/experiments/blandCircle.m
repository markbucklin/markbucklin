function code = blandRodeo
% blandCircle   Code for the ViRMEn experiment blandCircle.
%   code = blandCircle   Returns handles to the functions that ViRMEn
...
try
	...
	%original Colors
	vr.originalColor.P = w.surface.colors(2, vr.PTargetIndx);
	vr.originalColor.G = w.surface.colors(3,vr.GTargetIndx);
...
try
	%purple
	if (any(vr.vrSystem.distanceFromTarget.P < 0) == true)&(vr.targetOn.P == true)
