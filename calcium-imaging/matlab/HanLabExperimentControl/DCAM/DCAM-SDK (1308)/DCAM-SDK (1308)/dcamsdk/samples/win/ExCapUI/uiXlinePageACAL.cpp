...
/////////////////////////////////////////////////////////////////////////////
// CuiXlinePageACAL property page

IMPLEMENT_DYNCREATE(CuiXlinePageACAL, CPropertyPage)

CuiXlinePageACAL::CuiXlinePageACAL() : CExCapUIPropertyPage(CuiXlinePageACAL::IDD)
{
	//{{AFX_DATA_INIT(CuiXlinePageACAL)
...
void CuiXlinePageACAL::AddProperties()
{
	// TapCalibMethod
	...
	// TapCalibBaseDataMemory
	AddPropertyID(DCAM_IDPROP_TAPCALIB_BASEDATAMEMORY);
	uipParam[TapCalibBaseDataMemory].iComboBox	= IDC_XLINE_ACAL_TAPCALIBBASEDATAMEMORY_COMBO;
...
CuiXlinePageACAL::~CuiXlinePageACAL()
{
...
void CuiXlinePageACAL::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CuiXlinePageACAL)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP

}

void CuiXlinePageACAL::updateStatus()
{
	CString	str;
	double	dValue;
	char	text [1024];
...
void CuiXlinePageACAL::start_timer()
{
...
void CuiXlinePageACAL::end_timer()
{
	if(uTimer != NULL)
	{
		KillTimer(uTimer);
...
BOOL CuiXlinePageACAL::start_calibration(int CalibrattionType)
{
	// set capture mode
...
BOOL CuiXlinePageACAL::cancel_calibration(BOOL bUpdate) 
{
	ASSERT (m_hdcam != NULL);
...
BOOL CuiXlinePageACAL::check_calibration()
{
	ASSERT (m_hdcam != NULL);
...
void CuiXlinePageACAL::update_calibration()
{
	...
	if(nCalibrating == TYPE_NONE_CALIBRATION)
	{
		...
		if(uipParam[SensorModeACAL].value == DCAMPROP_SENSORMODE__AREA)
		{
			bEnableTap = FALSE;
			bEnableCom = FALSE;
			bEnableAI  = FALSE;
...
BEGIN_MESSAGE_MAP(CuiXlinePageACAL, CPropertyPage)
	//{{AFX_MSG_MAP(CuiXlinePageACAL)
	...
	ON_CBN_SELENDOK(IDC_XLINE_ACAL_TAPCALIBBASEDATAMEMORY_COMBO, OnSelendokXlscAcalTapcalibbasedatamemoryCombo)
	ON_CBN_SELENDOK(IDC_XLINE_ACAL_TAPCALIBMETHOD_COMBO, OnSelendokXlscAcalTapcalibmethodCombo)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CuiXlinePageACAL message handlers

BOOL CuiXlinePageACAL::OnInitDialog() 
{
	AddProperties();
...
void CuiXlinePageACAL::OnCancel() 
{
	if (nCalibrating != TYPE_NONE_CALIBRATION)
	{
		cancel_calibration ();
...
BOOL CuiXlinePageACAL::OnApply() 
{
	return CPropertyPage::OnApply();
}

void CuiXlinePageACAL::OnXlscAcalTapgaincalibrationButton() 
{
	if(nCalibrating == TYPE_NONE_CALIBRATION)
	{
...
BOOL CuiXlinePageACAL::OnKillActive() 
{
	if (nCalibrating != TYPE_NONE_CALIBRATION)
	{
		cancel_calibration ();
...
void CuiXlinePageACAL::PostNcDestroy() 
{
	if (nCalibrating != TYPE_NONE_CALIBRATION)
	{
		cancel_calibration ();
...
void CuiXlinePageACAL::OnTimer(UINT_PTR nIDEvent) 
{
...
void CuiXlinePageACAL::OnXlscAcalTapcalibgainEachButton() 
{
	CuiXlineModalADJU pDlg;

	getfromDCAM_gtvaluearray();

	long	ch = 1;					// single energy
	...
	pDlg.setfirst(0,pDlg.p_gtlow[0],0);
	...
	if(modal == IDOK){
		...
		if(ch == 1)
		{
			for(i=0;i<pDlg.m_gtmodules_max+1;i++)
			{
				...
				setvaluetoDCAM(MakeIDPROP_array(iProp_ch0,i), p_gtlow[i]);
			}
		}
	}
...
void CuiXlinePageACAL::OnKillfocusXlscAcalTapcalibgainAllEdit() 
{
	CString	str;
	double	dNewValue;
...
void CuiXlinePageACAL::OnDeltaposXlscAcalTapcalibgainAllSpin(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CuiXlinePageACAL::OnXlscAcalTapcalibdatamemoryButton() 
{
	// TapCalibDataMemory
...
void CuiXlinePageACAL::OnXlscAcalStoretapcalibdatamemoryButton() 
{
...
void CuiXlinePageACAL::OnXlscAcalTapcalibgainAllButton() 
{
	int32	iProp_ch0 = DCAM_IDPROP_TAPCALIB_GAIN;
	double	value;
	CString	str;
...
BOOL CuiXlinePageACAL::getfromDCAM_gtvalue()
{
	// TapCalibGain
...
BOOL CuiXlinePageACAL::getfromDCAM_gtvaluearray()
{
	// TapCalibGain
	...
	int32	bChannel;
	BOOL	bArrayRaram_ch0;
	double	d_numberofelement_ch0;
	long	numberofelement_ch0;

	// ch1
	bChannel = MakeIDPROP_ch(iProp_ch0, 0);
	if(bChannel == 0)
	{	// do not have channel
...
void CuiXlinePageACAL::update_EnableStatus()
{
	CWnd*	pWnd;
	...
	if(nCalibrating == TYPE_NONE_CALIBRATION)
	{
		dwEnable[iCountEnable] = IDC_XLINE_ACAL_TAPCALIBBASEDATAMEMORY_COMBO;
		iCountEnable++;
	...
	{
		dwDisable[iCountDisable] = IDC_XLINE_ACAL_TAPCALIBBASEDATAMEMORY_COMBO;
		iCountDisable++;
...
BOOL CuiXlinePageACAL::OnSetActive() 
{
	CheckSupportDCAMPROP();
	...
	if(uipParam[SensorModeACAL].value == DCAMPROP_SENSORMODE__AREA)
	{
		pButtonTap->EnableWindow(FALSE);
	}
	else
	{
		pButtonTap->EnableWindow(TRUE);
	}
...
BOOL CuiXlinePageACAL::PreTranslateMessage(MSG* pMsg) 
{
		...
		if (pMsg->wParam == VK_RETURN)
		{
			...
			switch(nDlgCtrlID)
			{
				...
				case IDC_XLINE_ACAL_TAPCALIBGAIN_ALL_EDIT:
					...
					break;
				case IDC_XLINE_ACAL_TAPCALIBBASEDATAMEMORY_COMBO:
					break;
				case IDC_XLINE_ACAL_TAPCALIBMETHOD_COMBO:
					break;
				case IDC_XLINE_ACAL_TAPCALIBDATAMEMORY_BUTTON:
					OnXlscAcalTapcalibdatamemoryButton();
					break;
				case IDC_XLINE_ACAL_STORETAPCALIBDATAMEMORY_BUTTON:
					OnXlscAcalStoretapcalibdatamemoryButton();
					break;
...
void CuiXlinePageACAL::OnSelendokXlscAcalTapcalibbasedatamemoryCombo() 
{
		// TapCalibBaseDataMemory
	int		nID = TapCalibBaseDataMemory;
	CComboBox* pCombo = (CComboBox*)GetDlgItem(uipParam[nID].iComboBox);
...
void CuiXlinePageACAL::OnSelendokXlscAcalTapcalibmethodCombo() 
{
	// TapCalibMethod
