...
#include "dcamprop.h"
#include "legacy/features.h"
#include "legacy/paramColor.h"
...
// {{{ _EXCAPBASE_SUPPORTS_

// ----------------------------------------------------------------

BOOL dcamex_getproperty_valuemax( HDCAM hdcam, long idprop, long& maxvalue )
{
	DCAM_PROPERTYATTR	attr;
	memset( &attr, 0, sizeof( attr ) );
	attr.cbSize = sizeof( attr );
	...
	if( ! dcam_getpropertyattr( hdcam, &attr )
	 || ( ( attr.attribute & DCAMPROP_TYPE_MASK ) != DCAMPROP_TYPE_LONG ) )
		return FALSE;

	maxvalue = (long)attr.valuemax;
	return TRUE;
}

BOOL dcamex_getproperty_valuemax( HDCAM hdcam, long idprop, double& maxvalue )
{
	DCAM_PROPERTYATTR	attr;
	memset( &attr, 0, sizeof( attr ) );
	attr.cbSize = sizeof( attr );
	...
	if( ! dcam_getpropertyattr( hdcam, &attr )
	 || ( ( attr.attribute & DCAMPROP_TYPE_MASK ) != DCAMPROP_TYPE_REAL ) )
		return FALSE;

	maxvalue = attr.valuemax;
	return TRUE;
}
// }}} _EXCAPBASE_SUPPORTS_
