classdef HamamatsuCamera < MatlabCompatibleCamera
  % ------------------------------------------------------------------------------
  % HamamatsuCamera
  % FrameSynx toolbox
  % 1/8/2009
  ...
  %
  % SYNTAX:
  % >> cameraObject = HamamatsuCamera;
  % or
  % >> cameraObject = HamamatsuCamera('propertyname1',value1,...)
  %
  % where propertyname can be any one of the following properties.
  %
  % HamamatsuCamera Properties:
  ...
  %
  % IMPORTANT:
  % Be sure to delete the HamamatsuCamera object at the end of a recording session
  ...
  methods % Constructor & Destructor
	 function obj = HamamatsuCamera(varargin)
		if nargin > 1
		  for k = 1:2:length(varargin)
			 obj.(varargin{k}) = varargin{k+1};
		  end
		end
		obj.default = HamamatsuCameraDefault; %subclass of DefaultFile class
	 end
	 function delete(obj)
		try
