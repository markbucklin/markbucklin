	 ...
	 function obj = DefaultFile(varargin)
		...
		if isempty(obj.textFileDirectory)
		  if ~exist('settingspath','file')
			 % Get a Settings Path from User and Write Function for Future in Settings Folder
			 fpath = uigetdir('F:\', 'Specify a Settings Folder');
			 fid = fopen(fullfile(fpath,'settingspath.m'),'w+t');
			 filecontents = 'function pathname = settingspath()\npathname = fileparts(which(''settingspath.m''));\n';
		  ...
		  if ~isdir(fpath)
			 if ~mkdir(fpath)
				fpath = fullfile(userpath,'Settings');
				mkdir(fpath);
				warning('DefaultFile:textFileDirectory',...
				  ['Settings Folder Set to:  ',fpath]);
			 end
		  end
		  if ~exist(fpath,'dir')
			 addpath(fpath);
		  ...
		  if ~exist('settingspath','file') ==2 % Create
			 fid = fopen(fullfile(fpath,'settingspath.m'),'w+t');
			 filecontents = 'function pathname = settingspath()\npathname = fileparts(which(''settingspath.m''));\n';
			 fprintf(fid,filecontents);
			 fclose(fid);
	 ...
	 function checkFile(obj)
		if exist(fullfile(obj.textFileDirectory,obj.textFileName),'file') ~= 2 % file does not exist
		  if exist(obj.textFileName,'file') == 2
			 ...
			 fprintf('%s will use default settings from the file:\n%s\n\n',...
				obj.className,...
			 ...
			 switch lower(userinput(1:6))
				case 'locate'
				  ...
				  if fname ~= 0
					 ...
					 succ = copyfile(fullfile(pathname,fname),obj.textFileDirectory);
					 if ~succ
						addpath(obj.textFileDirectory,'-end');
						savepath;
					 end
				  else
					 createNewFile(obj)
