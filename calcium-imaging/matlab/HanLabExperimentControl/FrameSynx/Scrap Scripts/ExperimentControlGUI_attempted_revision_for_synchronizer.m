classdef ExperimentControlGUI < hgsetget
  ...
  methods % Construct/Desctructor/Close
    function obj = ExperimentControlGUI(varargin)
      ...
      imaqreset
      instrreset
      obj.cameraSystem = CameraSystem.empty();
      obj.behaviorSystem = BehaviorSystem.empty();
      obj.systemSyncObj = SystemSynchronizer;
      defineDefaults(obj)
  ...
  methods % Building Functions
    ...
    function buildMainFigure(obj)
      try
        ...
        obj.camControl.camObjPan = uipanel(obj.mainFig,...
          'units','pixels',...
          'tag','camobjpanel',...
      ...
      switch src
        case obj.camControl.camObjPoplist
          if strcmp(choice,'other')
            specObj = inputdlg('Enter the name of the Camera class');
            set(src,'string',[specObj,obj.default.camObjList,{'other'}],...
              'value',1);
          end
          if ~isempty(obj.allCameraObj)
            for n = 1:numel(obj.allCameraObj)
              camclasses{n} = class(obj.allCameraObj{n});
            end
            if ~any(strcmp(camclasses,get(src,'string')))
              set(obj.camControl.camObjLoadButton,'value',0,'string','Load Camera');
            else
              set(obj.camControl.camObjLoadButton,'value',1,'string','Disconnect Camera');
            end
          end
  ...
  methods % Connect/Disconnect Component Functions
    ...
    function connectCam(obj,choice)
      ...
      dalsasettings = {'configFileDirectory',obj.configFileDirectory;...
        ...
        'gain',obj.gain;...
        'offset',obj.offset;...
        'previewFigure',obj.mainFig;...
        'previewImageObj',obj.mainControl.previewIm;...
        'gui',obj}';
      commonsettings = {'resolution',obj.resolution;...
      ...
      try
        if ~isempty(obj.cameraObj)...
          ...
          switch lower(choice)
            case 'dalsacamera'
              camobj = DalsaCamera(camsettings{:});%TODO: trigger settings/control
          ...
          obj.cameraSystem(numel(obj.cameraSystem)+1) = CameraSystem('cameraObj',camobj);
          obj.cameraObj = camobj; % adds to 'allCameraObj' but only first camera is added to 'cameraObj'
        end
    ...
    function disconnectCam(obj,choice)
      try
        if nargin < 2
          choicecam = ones(size(obj.cameraSystem));
        else
          choicecam = strcmp(choice, {obj.cameraSystem.cameraClass});
        end
        lastwarn('');
        warning('off','imaq:coreco:lowLevelError');
        if ~isempty(obj.cameraSystem(choicecam))...
            && isvalid(obj.cameraSystem(choicecam))
          if isrunning(obj.cameraObj)
            stop(obj.cameraSystem(choicecam))
          end
          delete(obj.cameraSystem(choicecam));
        end
        [~,id] = lastwarn;
    ...
    function connectSystems(obj)
      ...
      if ~isempty(obj.behaviorSystem) && ~isempty(obj.cameraSystem)
        set(obj.cameraSystem,'sessionPath',obj.savePath);
        set(obj.behaviorSystem,'sessionPath',obj.savePath);
        obj.systemSyncObj.register(obj.cameraSystem(1),'frame');
        obj.systemSyncObj.register(obj.behaviorSystem,'experiment','trial');
        if numel(obj.cameraSystem)>1
          for n = 2:numel(obj.cameraSystem)
            obj.systemSyncObj.register(obj.cameraSystem(n))
          end
        end
        set(obj.behaviorSystem,'sessionPath',obj.savePath)
        set(obj.cameraSystem,'sessionPath',obj.savePath)
        obj.systemSyncObj.synchronize();
        start(obj.behaviorSystem);
        start(obj.cameraSystem);
      end
      obj.cameraObj = obj.cameraSystem(1).cameraObj;
      obj.stimulusPresentationObj = obj.behaviorSystem.stimulusPresentationObj;
      obj.illuminationControlObj = obj.cameraSystem(1).illuminationObj;
    ...
    function runExperiment(obj)
      ...
      figure(obj.mainFig)
      % 						if ismethod(obj.cameraObj,'stoppreview')
      % 								stoppreview(obj.cameraObj)
      % 						end
      obj.connectSystems;
  ...
  methods % Set Functions
    ...
    function set.cameraObj(obj,camobj)
      % Add first camera created to 'cameraObj' prop and add all others to allCameraObj cell
      % array
      if ~isempty(obj.allCameraObj)
        newcam = true;
        for n = 1:numel(obj.allCameraObj)
          if camobj == obj.allCameraObj{n}
            newcam = false;
          end
        end
        if newcam
          obj.allCameraObj{n+1} = camobj;
        end
      else
        obj.allCameraObj{1} = camobj;
