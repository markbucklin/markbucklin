classdef ImageAcquisitionGUI < hgsetget
		...
		methods % Construct/Desctructor/Close
				function obj = ImageAcquisitionGUI(varargin)
						if nargin > 1
						...
						addlistener(obj.dataGeneratorObj,'NewStimulus',@(src,evnt)newStimFcn(obj,src,evnt));
						addlistener(obj.dataGeneratorObj,'NewTrial',@(src,evnt)newTrialUpdateFcn(obj,src,evnt));
						buildMainFigure(obj)
						setFigureAccess(obj)
...
% 										'string','Movie-Sequence Options');
		...
		methods (Hidden)% Control Functions
				...
				function trigAverageControlFcn(obj,src,evnt) % ???
						warning(me.message);disp(me.stack(1)) % What the hell is this function?
...
% 								colormap(obj.mainAx,cmap);
		...
		methods % Connect/Disconnect Component Functions
				...
				function disconnectCam(obj)
						if obj.debugMode
								answer = questdlg('Disconnect Camera?');
								if ~strcmpi('yes',answer)
										return
								end
						end
				...
				function connectIllumination(obj)
						...
						obj.illuminationControlObj = IlluminationControl(...
								'channelSequence',obj.channelSequence);
						set(obj.illumControl.illumOnButt,'value',1,'string','Turn Off')
						set(obj.illuminationControlObj,'operationMode','listen');
...
% 						end
						...
						activate(obj.dataGeneratorObj);
				end
				function interruptExperiment(obj)
						deactivate(obj.dataGeneratorObj)
						obj.exptRunning = false;
				end
		end
