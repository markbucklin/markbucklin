classdef (CaseInsensitiveProperties, TruncatedProperties) VideoStreamProcessor < ignition.core.Module & matlab.System
% VideoStreamProcessor - Interface to 'matlab.System' type module
	...
	% PRIVATE/INTERNAL
	% ##################################################
	properties (Nontunable, Access = ?ignition.core.Module, Hidden)
	...
	% ##################################################
	methods (Access = ?ignition.core.Module)
		function F = checkInput(obj, F)
			
			if isnumeric(F)
				checkNumericInput(F)
				
			elseif isa(F,'ignition.shared.datatype.VideoSegment')
			...
			% DATA-TYPE
			obj.InputDataType = ignition.shared.getPixelDataType( F);
	...
	methods (Access = protected)
		...
		function varargout = stepImpl(obj, F, varargin)
			try
				...
				addBenchmark(obj.BenchmarkObj, procTime, getNumFrames(F));
