function varargout = imrc(im, backgroundColor)
warning('imrc.m being called from scrap directory: Z:\Files\MATLAB\toolbox\ignition\scrap')

if nargin < 2
	backgroundColor = [0 0 0];
	% 	backgroundColor = [.5 .5 .5; 0 0 0; 1 1 1];
end
im = squeeze(im);
nFrames = size(im,3);
if nFrames > 1
...
nTotal = numel(unique(im));
if nTotal<=1024;	
	cmap = distinguishable_colors(nTotal, backgroundColor);
else
	cmapRep = distinguishable_colors(1024, backgroundColor);
	numReps = min(16, fix(nTotal/1024));
	cmap = repmat(cmapRep, numReps,1);
end
% cmap = cat(1, backgroundColor(1,:), cmap);
cmap = cat(1, backgroundColor, cmap);
