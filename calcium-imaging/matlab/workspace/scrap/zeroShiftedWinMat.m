function Xbf = zeroShiftedWinMat(x,Mf,Mb)
warning('zeroShiftedWinMat.m being called from scrap directory: Z:\Files\MATLAB\toolbox\ignition\scrap')
...
if (size(x,2) > 1) && (size(x,1) > 1)
   ...
   for k = 1:size(x,2)
	  Xbf(:,:,k) = zeroShiftedWinMat(x(:,k), Mf, Mb);
   end
   return
end
M = max(Mf, Mb);
