...
[tiffLoader.config,tiffLoader.control,tiffLoader.state] = ignition.io.tiff.initializeTiffFileStream();
end


pp.env.numVideoFrames = tiffLoader.config.numFrames;
assignin('base','tiffLoader',tiffLoader);
...
% 	videoFile = scicadelic.FileWrapper('FileName',TL.FileName, 'FileDirectory', TL.FileDirectory);
	pp.env.defaultDataSetName = tiffLoader.config.fileInputObj.DataSetName;
% catch
% 	pp.env.defaultDataSetName = TL.DataSetName;
% end
pp.env.defaultExportPath = fullfile(tiffLoader.config.fileInputObj.FileDirectory, 'export');
...
allSystems = cat(1,feedThroughSystem,otherSystem);

%% PREALLOCATE VARIABLES & DATA-OUTPUT ARRAYS

% SIZE/DIMENSION
pp.env.numRows = tiffLoader.config.numRows;
pp.env.numCols = tiffLoader.config.numCols;
N = tiffLoader.config.numFrames;
pp.env.numFrames = N;
...
%     end
	function chunk = next()
		if ~finishedFlag
			...
			[chunk.normalizedSelectedStatistics.float,chunk.normalizedSelectedStatistics.uint8RGB] = generateColorCodedMarginalStatChannels();
			if saveRGB
				saveRGBMarginalStatFile(chunk.normalizedSelectedStatistics.uint8RGB);
			end
			if saveBW
			...
			if strcmp(questdlg('Release and delete all systems?'),'Yes')
				...
				try
					...
					chunk.data = [];
					chunk.info = [];
					chunk.pixelStatistics = [];
					chunk.normalizedSelectedStatistics.uint8RGB = [];
					chunk.normalizedSelectedStatistics.float = [];
				end
			end
		end
	end
	function varargout = processChunk()
		...
		% LOAD ------------------------------------
		if tiffLoader.state.StreamFinishedFlag || (idx(end) >= stopIdx)
			F = [];
			idx = [];
			frameTime = [];
			finishedFlag = true;
            ...
            idx = idx(end) + (1:framesPerStep)
% 			[frameData, frameInfo, frameIdx] = step(TL);
            [videoFrame, tiffLoader.state.StreamFinishedFlag, tiffLoader.state.CurrentFrameIdx] = ignition.io.tiff.readTiffFileStream( tiffLoader.config, idx);
			frameInfo = cat(1, videoFrame.Info);
            frameData = cat(3,videoFrame.Data);
            frameTime = [frameInfo.TimeStamp];
            frameIdx = [frameInfo.FrameNumber];
		end

		F = single(squeeze(gpuArray(frameData)));
% 		idx = tiffLoader.state.CurrentFrameIdx;
		...
		pp.chunkInfo(m).idx = tiffLoader.state.CurrentFrameIdx(:);
...
% RGB DATA OUTPUT
	...
	function updateWorkspacePPStruct()
		try
            if (evalin('caller','exist(''pp'',''var'')'))
                assignin('caller','pp',pp)
            end
        catch
            if (evalin('base','exist(''pp'',''var'')'))
                disp('updating pp structure in base workspace')
                assignin('base','pp',pp)
            end
        end
	end
