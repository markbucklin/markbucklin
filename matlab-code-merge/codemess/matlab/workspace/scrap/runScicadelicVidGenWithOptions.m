function [allInfo, scicadelicPathCleaner] = runScicadelicVidGenWithOptions(TL, outputFormat)

% Specify Output Options (todo)
if nargin < 2
    outputFormat = 'mp4';
end
outputOptions = {'mp4','avi','bin','none','var','dbg'};
assert(any( cellfun(@(op)strncmpi(outputFormat, op, 2), outputOptions)));
...
try
    ...
    writeRawFrgb = strncmpi(outputFormat,'binary',2);
    % if nargout > 1
    % 	writeRawFrgb = true;
    % else
    % 	writeRawFrgb = true;
    ...
    videoFileNameRoot = [defaultExportPath,filesep, defaultDataSetName, dateStamp];
    
    % Video Output File Specifications (AVI or Mp4 so far)
    vidOutFPS = 24;
    writeVidOut = true;
    if strncmpi(outputFormat,'mp4', 2)
        vidOutFileName = [videoFileNameRoot, '.mp4'];
        vidOutProfile = 'MPEG-4';
        vidOutSetting = {'Quality',98};
    elseif strncmpi(outputFormat,'avi', 2)
        vidOutFileName = [videoFileNameRoot, '.avi'];
        vidOutProfile = 'Uncompressed AVI';
        vidOutSetting = {};
    else
        writeVidOut = false;
    end
    ...
    if writeVidOut
        ...
        writerObj = VideoWriter( vidOutFileName, vidOutProfile);
        vidOutSetting = [{'FrameRate', vidOutFPS}, vidOutSetting];
        set(writerObj, vidOutSetting{:});
        addlistener(writerObj, 'ObjectBeingDestroyed', @(src,evnt)close(src) );
        open(writerObj)
        % Create Timer to Write Data Asynchronously
        
        batchWriteFcn = @(frames)writeVideo(writerObj, frames);
    end
    ...
    s0 = [];
    sa = [];
    allInfo = cell(1,ceil(N/numFrames));
        ...
        try
            ...
            try
                ...
                if idx(1) > 4
                    ...
                    fRGB = zeros(numRows,numCols,3,numFrames,'uint8');
            ...
            % Write Frames (attempt to write async first)
            if writeVidOut
                
                try
                    fevalAsync( batchWriteFcn, 0, fRGB);
                catch
            ...
            % WRITE BINARY FILE
            if writeRawFrgb
                binaryFileName = writeBinaryData(fRGB, videoFileNameRoot, false);
                %                 fprintf('Frames written to binary file: %s\n',binaryFileName)
            end
