...
warning('addScrapWarning.m being called from scrap directory: Z:\Files\MATLAB\toolbox\ignition\scrap')
warning('addScrapWarning.m being called from scrap directory: Z:\Files\ignition\ignition\scrap')
...
for k=1:numel(allFiles)
	...
	try
		...
		switch fileType
			...
			case 'class'
				className = fileCallInfo(1).name;
				classConstructorInfo = fileCallInfo( ...
					strcmp({fileCallInfo.name},className) ...
					& strcmp({fileCallInfo.type},'class method') );
				entryLine = classConstructorInfo(1).firstline;
				warnLine = entryLine + 1;
				if ~strcmp(currentFileText{warnLine}, warningString)
					newFileText = cat(1, currentFileText(1:entryLine), {warningString}, currentFileText(warnLine:end) );
				end
			otherwise
		end
