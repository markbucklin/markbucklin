function [nextFcn, varargout] = getScicadelicPreProcessor(saveBW,saveRGB)
% [nextFcn,pp] = getScicadelicPreProcessor(saveBW,saveRGB);
...
%%
    ...
    function [f,info,mstat,frgb,srgb] = next()
        if ~finishedFlag
           ...
           [frgb,srgb] = generateColorCodedMarginalStatChannels();
           if saveRGB
...
% RGB DATA OUTPUT
    function [fRGB, varargout] = generateColorCodedMarginalStatChannels()
        ...
        try
            ...
            if idx(1) > 4
                ...
                fRGB = zeros(pp.env.numRows,pp.env.numCols,3,numFrames,'uint8');
