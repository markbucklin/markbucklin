...
% Exports the graph to DOT file format that can be used with AT&T GraphViz or Gephi to plot the graph
%
% Receieves:
...
%
% See Also:
%   ObjectCreateGraph,LoadGraph, GraphLoadGraphLayoutPlain, GraphPlotGraphLayout
%   
% Algorithm:
%   For detailed documentation see: http://www.graphviz.org/content/dot-language
% 
% Example:
...
if isfield(Graph,'Index') && ~isempty(Graph.Index.Values)
    ...
    for Node = 1 : numel(NodeIDs)
        DOT{end+1} = [ num2str(NodeIDs(Node)) ' [label="' num2str(NodeIDs(Node)) '" shape=circle ];'];        % ellipse - box, circle, record and plaintext
    end
    DOT{end+1} = [ '// Nodes end'];
end
DOT{end+1} = [ '// Links start'];
for Link = 1 : size(Graph.Data,1)
   DOT{end+1} = [ num2str(  Graph.Data(Link,1) ) EdgeConnector num2str(  Graph.Data(Link,2) ) ' [' Attributes ' spline="line" ];' ];
end
DOT{end+1} = [ '// Links end'];
