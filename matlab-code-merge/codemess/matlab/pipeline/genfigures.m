...
%% SHOW ROIs
h = show(R);
print(h.fig, fullfile(figDir,'Regions of Interest'), '-dpng','-r450')
...
print(gcf,fullfile(figDir,get(get(gca,'Title'), 'String')), '-dpng','-r450')
...
for kRoi = 1:nRoi
   ...
   if any(k==exampleRoiIdx)
	  ...
	  print(gcf,fullfile(figDir, sprintf('Tone-Triggered Trace Example Neuron %i', k)), '-dpng','-r450')
	  clf
   end
end
...
imagesc(-1:(1/20):5,[], normsig.z(squeeze(ft6( :, :, size(ft6,3))))')
xlabel('Time (s) - Sound onset at t=0')
ylabel('Trial')
title(sprintf('Tone-Triggered Trace (Both) Example Neuron %i', size(ft6,3)))
print(gcf, fullfile(figDir, sprintf('Tone-Triggered Trace (Both) Example Neuron %i', size(ft6,3))), '-dpng','-r450')
...
for boxfiltsize = 0:5:20
   for k=0:4
	  ...
	  print(gcf,fullfile(figDir,figname), '-dpng','-r450')
	  clf
   end
end
...
print(gcf,fullfile(figDir,'Trial-Triggered Mean'), '-dpng','-r450')
clf
...
print(gcf,fullfile(figDir,'Lick-Triggered-Lick'), '-dpng','-r450')
clf
subplot(3,1,1)
...
xlabel('Time - 0 = tone onset')
ylabel('Trial')
hano = annotation('textbox','Position',[.01 0 .98 .05],'String','Sound-Triggered-Lick');
hano.EdgeColor = 'none';
hano.FitBoxToText = 'on';
hano.LineStyle = 'none';
hano.VerticalAlignment = 'bottom';
print(gcf,fullfile(figDir,'Sound-Triggered-Lick'), '-dpng','-r450')
clf
...
print(gcf,fullfile(figDir,'Lick-Triggered-Mean'), '-dpng','-r450')
clf
...
for k = 1:nRoi
   ...
   if any(k==exampleRoiIdx)
	  ...
	  print(gcf,fullfile(figDir,sprintf('First-Lick-Triggered Trace - Example Neuron %i', k)), '-dpng','-r450')
   end
end
...
print(gcf,fullfile(figDir,'Lick-Triggered-Mean (Pre-Normalized)'), '-dpng','-r450')
close all
...
kroi = 1;
for k=1:size(ft6,2)
  hline(k) = line(t_ft6, squeeze(ft6(:,k,kroi)),'Color',[0 0 1 .2],'Parent',ax, 'LineWidth',.5); 
end
hmeanline = line(t_ft6, squeeze(nanmean(ft6(:,:,kroi),2)),'Color',[.95 .95 0 .3], 'Parent',ax, 'LineWidth',20); 
title('Tone-Triggered Traces for Example ROI 1 (BLUE) with Mean (YELLOW)')
xlabel('Time (s) - Sound onset at t=0')
print(gcf, fullfile(figDir,'Tone-Triggered-Traces (single ROI) with ROI-Mean'), '-dpng','-r450')