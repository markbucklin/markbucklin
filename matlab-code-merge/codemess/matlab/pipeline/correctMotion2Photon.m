function [data, xc, prealign] = correctMotion2Photon(data, prealign)
...
maxOffset = floor(min(cropSize(1:2))/6);
...
if ~isfield(prealign, 'template')
   vidMean = single(croppedVid(:,:,1));
%    templateFrame = vidMean(ysub,xsub);
templateFrame = vidMean;
else
...
for k = 1:N
   ...
   movingFrame = single(croppedVid(:,:,k));
...
[yoffset, xoffset] = fastreg(templateFrame, movingFrame);
   if abs(yoffset) >= maxOffset
	  yoffset = gpuArray(dy);
   end
   if  abs(xoffset) >= maxOffset
	  xoffset = gpuArray(dx);
   end
adjustedFrame = padarray(movingFrame, [maxOffset, maxOffset], 0, 'both');
...
%    padFrame = padarray(gpuArray(data(:,:,k), [maxOffset maxOffset], 'replicate', 'both');
%    data(:,:,k) = padFrame(yPadSub+dy, xPadSub+dx);
% data(:,:,k) = gather(interp2(padFrame, Xq, Yq));
data(:,:,k) = gather(adjustedFrame);
end
