...
% tally.centroid = cellfun(@numel, part.bycentroid{1});
% tally.xylim = cellfun(@numel, part.bylimit{1});
nComRoi = 0;
for kSize = 1:numel(part.bysize)
  %   p.bycentroid = part.bycentroid{kSize};
  p.bylimit = part.bylimit{kSize};
  sz = size(p.bylimit);
  groupedRois = cell(sz);
  cnum = cellfun(@numel, p.bylimit);
  for ky = 1:sz(1)%TODO: go by every other frame and check if subregion -> remove
    for kx = 1:sz(2)
      if cnum(ky,kx) > minPartitionIncidence        
        ...
        while any(groupIdx)
          ...
          %           imshow(cat(3, foAll, imcomplement(csAll./max(csAll(:))), double(ovlp)))
          % Sort the group
          nGroups = nGroups + 1;
