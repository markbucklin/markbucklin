...
% 						delete(subRoi);
	...
	methods % PRIVATE MANAGEMENT FUNCTIONS
		function varargout = guessFrameSize(obj)
			maxCentroid = max(cat(1,obj.Centroid));
		...
		function roiClickFcn(obj,src,evnt)
			...
			if isa(src,'matlab.graphics.primitive.Patch')
...
% 					roiShowingTrace(k) = true;
			...
			if isempty(roiShowingTrace)
				roiShowingTrace = RegionOfInterest.empty(1,0); %false(numel(obj),1);
			end
			switch evnt.Button;
...
% 					roiShowingTrace = false(numel(obj),1);
				case 3 % RIGHT-BUTTON: PLOT MULTIPLE TRACES
					if ~isempty(selectedObj)
						for k = 1:numel(selectedObj)
							if ~isempty(roiShowingTrace)
								alreadyShowing = (selectedObj(k) == roiShowingTrace);
							else
								alreadyShowing = false;
							end
							if any(alreadyShowing)
								% Remove Plot & Line
								thisLine = findobj(hLine, 'Color', selectedObj(k).Color);
								if~isempty(hLine)
									delete(hLine)
								end
								thisText = findobj(hTx, 'Color', selectedObj(k).Color);
								if~isempty(thisText)
									delete(thisText)
								end
								roiShowingTrace = roiShowingTrace(~alreadyShowing);
							else
							% Add Plot
							...
							hTx(numel(hTx)+1) = handle(text(...
								...
								'Position', round(selectedObj(k).BoundingBox(1:2)) - [0 5],...
								'Parent', src.Parent));
							roiShowingTrace = cat(1, roiShowingTrace(:), selectedObj(:));
							% 							roiShowingTrace(k) = true;
