function roi = generateSingleFrameRois(data,info)
% INPUT:
...
signalThreshold = gpuArray( stat.Min + cast( stat.Std.*stdOverMin, 'like', data ));
...
% FUNCTION TO MAKE BINARY MASK WITH ADAPTIVE THRESHOLD
   function [bw, sigThresh]  = getAdaptiveHotspots(diffImage, sigThresh)
	  ...
	  if depth > recursionLim
		 ...
		 sigThresh = gpuArray( stat.Min + cast( stat.Std.*stdOverMin , 'like', data ));% NEW, (reset)
		 return
	  end
