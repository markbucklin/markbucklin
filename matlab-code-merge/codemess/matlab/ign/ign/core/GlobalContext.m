classdef (CaseInsensitiveProperties, TruncatedProperties, ConstructOnLoad) GlobalContext < handle
...
% prodStructFinal = matlab.internal.prodcheck()	
	
	
	
	methods
		function obj = GlobalContext()
			%TODO add varargin
			fprintf('GlobalContext: Constructor\n')
			
			global GLOBAL_CONTEXT
			
			if isempty(GLOBAL_CONTEXT) || ~isvalid(GLOBAL_CONTEXT) %|| isempty(obj.NumRefs)
		...
		function initialize(obj)						
			...
			fprintf('GlobalContext: Initialize\n')									
			try
				...
				if isempty(obj.PackageName)
					pkg = inputdlg('Type the package to use','GlobalContext - Current Package', 'ignition');
				...
				% todo -> use superuseful >> env = dct_psfcns('environ');
		...
		function disconnect(obj)
			...
			fprintf('GlobalContext: Disconnect\n')
	...
	methods (Static)
		function reset()
			global GLOBAL_CONTEXT
			fprintf('GlobalContext: Reset\n')
			
			try
				if ~isempty(GLOBAL_CONTEXT) && isvalid(GLOBAL_CONTEXT)
					obj = GLOBAL_CONTEXT;
					disconnect(obj)
					...
					GLOBAL_CONTEXT = ignition.core.GlobalContext;
				end								
			catch
				GLOBAL_CONTEXT = [];
			end
	...
	methods
		function delete(obj)
			obj.IsConnected = false;
			% 			global GLOBAL_CONTEXT
			% 			fprintf('GlobalContext: Delete\n')
			
			% DECREMENT REFERENCE/CALL COUNTER
			% 			obj.NumRefs = obj.NumRefs - 1;
			% 			if obj.NumRefs < 1
			% 				GLOBAL_CONTEXT = ignition.core.GlobalContext.empty;
			% 			end
		end
	end
