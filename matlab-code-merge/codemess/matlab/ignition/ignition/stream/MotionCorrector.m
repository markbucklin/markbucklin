classdef  (CaseInsensitiveProperties = true) MotionCorrector <  ignition.core.VideoStreamProcessor
	...
	methods (Access = protected)
		function setupImpl(obj,data,~)
			...
			% 			obj.FixedFrameStatCollector = ignition.StatisticCollector;
			% 			obj.CorrCoeffStatCollector = ignition.StatisticCollector;
			setPrivateProps(obj)
			...
			if isempty(obj.FrameCorrelationPixelWeight)
				obj.FrameCorrelationPixelWeight = ignition.shared.onGpu( ones(numRows, numCols, 'single'));
			end
	...
	methods (Access = protected)
		function Uxy = findFrameShift(obj, movingInput, fixedInput)
			...
			frameSubs = ignition.shared.onGpu( single(reshape(1:numFrames, 1,1,numFrames)));
		...
		function info = getCorrectionInfo(obj, Uxy)
			...
			dUxy = ignition.shared.onCpu( bsxfun(@minus, Uxy , UxyBuf(1:end-1,:))); % Uxy(:,:,1:end-1))));
			Uxy = ignition.shared.onCpu( Uxy);
			C = ignition.shared.onCpu( obj.FrameCorrelationUnweightedOutput(:));
			Cw = ignition.shared.onCpu( obj.FrameCorrelationWeightedOutput(:));
			bStable = ignition.shared.onCpu( obj.StableFrameMostRecent(:));
	...
	methods (Access = protected, Hidden)
		function setupRigidRegistration(obj)
			...
			obj.SubWinRowSubs = ignition.shared.onGpu( subsCenteredOn(obj.SubWinCenterRow, subWinSize(1)));
			obj.SubWinColSubs = ignition.shared.onGpu( subsCenteredOn(obj.SubWinCenterCol, subWinSize(end)));
			
			% CONSTRUCT WINDOWING FUNCTION/MASK TO REDUCE EDGE-EFFECTS OF FFT
			% 			obj.SubWinAntiEdgeWin = ignition.shared.onGpu( single(hann(subWinSize) * hann(subWinSize)'));
			obj.SubWinAntiEdgeWin = ignition.shared.onGpu( single(hann(subWinSize(1)) * hann(subWinSize(end))')); %TODO
