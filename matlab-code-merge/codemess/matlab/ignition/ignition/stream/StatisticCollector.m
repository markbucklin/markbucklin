classdef (CaseInsensitiveProperties = true) StatisticCollector < ignition.core.VideoStreamProcessor
	% StatisticCollector
	...
	methods (Access = ?ignition.core.Object)
		function varargout = runMainTask(obj, F)
		...
		function updateStatistics(obj, F)
			...
			if size(F,frameDim) >= 1
				...
				if obj.UseGpu					
					...
					% -----------------------------------
					stat = ignition.stream.gpu.updateStatisticsGPU(F, stat);
		...
		function dstat = getDiffMomentWithStatisticUpdate(obj, F)
			...
			if size(F,frameDim) >= 1
				...
				if obj.UseGpu					
					...
					% GPU VERSION (EXTERNAL)
					% -----------------------------------																	
					[dstat, stat] = ignition.stream.gpu.getStatisticDifferentialGPU(F, stat);
	...
	methods (Access = public)
		function stat = getStatistics(obj, leaveOnGpu)
			...
			if leaveOnGpu || ~obj.UseGpu
				...
				stat = struct(...
					'N', obj.N,...
					'Min', ignition.shared.onCpu( obj.Min),...
					'Max', ignition.shared.onCpu( obj.Max),...
					'Mean', ignition.shared.onCpu( obj.Mean),...
					'StandardDeviation', ignition.shared.onCpu( obj.StandardDeviation),...
					'Variance', ignition.shared.onCpu( obj.Variance),...
					'Skewness', ignition.shared.onCpu( obj.Skewness),...
					'Kurtosis', ignition.shared.onCpu( obj.Kurtosis),...
					'JarqueBera', ignition.shared.onCpu( obj.JarqueBera));
			end
		end
...
% ##################################################
% TODO: reverse order of input arguments so that stat can be instance of object
