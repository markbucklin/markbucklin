function [stackInfo, fileInfo, allTiffObj] = initializeTiffStack(tiffFileInput, parseFrameInfoFcnHandle)
%
%
%		parseFrameInfoFcn
...
if (nargin < 2)
	...
	parseFrameInfoFcnHandle = defaultParseFcn;
	if (nargin < 1)
		tiffFileInput = [];
	end
end
if isempty(tiffFileInput)
	...
	tiffFileInput = ignition.io.FileWrapper('FileExtension','tif'); %selectTiffFiles();
...
if isempty(fileInputObj)
	...
	fileInputObj = ignition.io.FileWrapper('FileName', fname, 'FileDirectory', fdir, 'FileExtension',fext);
...
stackInfo.fileInputObj = fileInputObj;
...
% ----------------------------------------------
[frameTime, frameInfo] = parseFrameInfoFcnHandle(firstTiffObj);
...
for kFile = 1:numFiles
	...
	% RECORD FIRST FRAME TIME & INFO (FRAME-METADATA)
	[frameTime, frameInfo] = parseFrameInfoFcnHandle(tiffObj);
...
stackInfo.parseFrameInfoFcnHandle = parseFrameInfoFcnHandle;
