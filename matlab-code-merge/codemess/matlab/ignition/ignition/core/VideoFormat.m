classdef (CaseInsensitiveProperties, TruncatedProperties, HandleCompatible)...
	...
	% todo!!!!!!!!!
	% Bonsai (.NET) has following properties
	%
	% FrameSize
	% AverageFrameRate
	% MaximumFrameRate
	% BitCount
	%
	% ---> Attach to any Video Source ( i.e. file, capture, processor output)
	%
	...
	% ------->> todo: create a VideoFormat class (handle, 1 per stream)
	
	
	% CONSTRUCTOR
	methods
		function obj = VideoFormat(varargin)
			% >> obj = VideoFormat(frameData)
			% >> obj = VideoFormat(frameData, frameInfo)
			% >> obj = VideoFormat(frameData, frameInfo, frameTime)
			% >> obj = VideoFormat(frameData, frameInfo, frameTime, frameIdx)
			% >> obj = VideoFormat(frameData, frameTime, frameInfo, frameIdx)
			...
			if nargin
				...
				if isa(argsIn{1}, 'ignition.core.type.VideoFormat')					
		...
		function refObj = getReference(obj)
			% todo
		end
	end
	...
	methods (Static)
		function obj = buildFromFullInput(className, allData, allInfo, allTimestamp, allIdx)
			...
			% 			obj(N,1) = ignition.core.type.VideoFormat();
			...
			% 				% 					frameObj(k) = ignition.core.type.VideoFormat( data, info, timestamp, idx);
			% 				% 							if isHandleObj
			% 				% 								lastObj = frameObj(k);
			% 				% 							end
		end			
...
% 			try
% 				if ~isa(vidObj, 'ignition.core.type.VideoFormat')
% 					obj = ignition.core.type.VideoFormat();
...
% 				
% 				obj = ignition.core.type.VideoFormat.splitFromOrderedInput(allData,allTimestamp,allInfo,allIdx);
% 				% 						N = numel(allIdx);
% 				% 						frameObj(N,1) = ignition.core.type.VideoFormat();
% 				% 						for k = 1:N %N:-1:1
...
% 				%
% 				% 							% PASS TO CONSTRUCTOR (IN VIDEODATA PARENT CLASS)
% 				% 							frameObj(k) = ignition.core.type.VideoFormat( data, timestamp, info, idx );
% 				%
% 				% 							% 							if isHandleObj
% 				% 								% 								lastObj = frameObj(k);
% 				% 								% 							end
