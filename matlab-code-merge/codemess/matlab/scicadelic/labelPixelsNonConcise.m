function [pixelLabel, pixelLayer, labelAccumStats, labelSize, pixelLabelLocked, labelIncidence, pixelLabelSteady] ...
	= labelPixels(pixelIntensity, pixelLabel, pixelLayer, pixelLabelLocked, labelIncidence, radiusRange, numRegionalSamples, bThresh)
...
% ============================================================
% persistent Lcnt
[numRows, numCols, numFrames] = size(pixelIntensity);
...
if (nargin < 8) || isempty(bThresh)
	bThresh = fix(min(range(pixelIntensity,1),[],2) / 4);
end
...
pixelLayerUpdate = sum(...
	...
	pixelIntensity, rowSubs, colSubs, frameSubs, bThresh, radiusSample),...
	4) ./ actualNumSamples;
...
% ============================================================
[labelAccumStats, labelSize] = getEncodedLabelSizeMeanMaxMin(pixelLabel, pixelIntensity);
...
% ============================================================
	function upPx = updatePixelLayerLikelihood(fPx, rowC, colC, n, b, r)
		...
		fPxUL = single(pixelIntensity(rowU, colL, n));
		fPxUC = single(pixelIntensity(rowU, colC, n));
		fPxUR = single(pixelIntensity(rowU, colR, n));
		fPxDL = single(pixelIntensity(rowD, colL, n));
		fPxDC = single(pixelIntensity(rowD, colC, n));
		fPxDR = single(pixelIntensity(rowD, colR, n));
		fPxCL = single(pixelIntensity(rowC, colL, n));
		fPxCR = single(pixelIntensity(rowC, colR, n));
