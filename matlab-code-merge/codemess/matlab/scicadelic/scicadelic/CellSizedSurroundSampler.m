classdef (CaseInsensitiveProperties = true) CellSizedSurroundSampler < scicadelic.SciCaDelicSystem
	% CellSizedSurroundSampler
	%
	% INPUT:
	%
	% OUTPUT:
	...
	%			CellDetector (former)
	%			PixelPopper (former)
	%			CellSizedSurroundSampler (current)
	%			PixelClusterSurroundComparator
	%			NeighborhoodSurroundComparator
	%			ClusteredCellSizedPixelPlateauGenerator
	...
	% CONSTRUCTOR
	methods
		function obj = CellSizedSurroundSampler(varargin)
	...
	methods (Access = protected)
		function setupImpl(obj, data)
			fprintf('CellSizedSurroundSampler -> SETUP\n')
		...
		function output = stepImpl(obj, data)
			...
			% THRESHOLDING PROCEDURE THAT FINDS CELL-SIZE CLUSTERS OF BRIGHT PIXELS
			data = onGpu(obj, data);
	...
	methods (Access = protected)
		function bwF = processData(obj, F, maxExpectedDiameter)
			...
			% TODO: for multiple diameters
			
			maxD = maxExpectedDiameter(1);
			
						
			% EACH PIXEL WILL SAMPLE SURROUD AT DISTANCE MAX-EXPECTED DIAMETER AND SMALLER POWERS-OF-2
			% ( e.g. if input is 10 -> dsVec = [4 8 10]  -> changed to [3 7 10]
			if maxD > 8
				dsVec = [2.^(2:nextpow2(maxD)-1) - mod(maxD, 2)+1 , maxD];
			else
				dsVec = maxD;
			end
			...
			% TODO: use SPMD with gop(@or, nearNeighborDominanceArrayWise(F, ds)
		end
	end
