classdef (CaseInsensitiveProperties = true) CellDetector < scicadelic.SciCaDelicSystem
	% CellDetector
	%
	% INPUT:
	%
	...
	% CONSTRUCTOR
	methods
		function obj = CellDetector(varargin)
	...
	methods (Access = protected)
		function setupImpl(obj, F)
			...
			if nPastFrames >= 1
				bwFg = findPixelForeground(obj, F, obj.MaxExpectedDiameter);
			...
			obj.OutputTypeIdx = getIndex(obj.OutputTypeSet, obj.OutputType);
			% 			outputSample = processData(obj, data); if isa(outputSample, 'gpuArray')
	...
	methods (Access = protected)
		function output = processData(obj, F)
			...
			lm = findSegmentedRegions(obj, bwFg);
	...
	methods (Hidden)
		...
		function bw = testMorphOpFcn(obj, bw)
			...
			% 			fprintf('%s\n', func2str(fcn))
			% 			else
			% 			elseif isnumeric(opSpec)
			% 				return
			% 				bw = bwmorph(bw, opName);
			% 			end
		end
	end
