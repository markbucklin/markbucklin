classdef (CaseInsensitiveProperties = true) BinPixelTemporalFilter < scicadelic.SciCaDelicSystem
	% BinPixelTemporalFilter
	%
	% INPUT:
	%
	% OUTPUT:
	...
	properties (Access = public, Nontunable)
		FilterNumFrames = 1;		% Number of prior frames to filter with current frame
		FilterType = 'Recurrence' % 'Recurrence' , 'And' ('All') , 'Or' ('Any')
	end
	...
	properties (SetAccess = protected, Hidden)		
		InputBuffer					% Logical array 'FilterNumFrames' frames
	end
	
	% PRIVATE VARIABLES
	properties (SetAccess = protected, Hidden, Nontunable)
		FilterTypeSet = matlab.system.StringSet({'Recurrence','And','All','Or','Any'})
		FilterTypeIdx
	end
	properties (SetAccess = protected, Hidden, Nontunable)
		pFilterNumFrames
	...
	% CONSTRUCTOR
	methods
		function obj = BinPixelTemporalFilter(varargin)
	...
	methods (Access = protected)
		function setupImpl(obj, data)
			fprintf('BinPixelTemporalFilter -> SETUP\n')
			...
			bwF = data;
			m = obj.FilterNumFrames;
			if m >= 1				
				if m <= size(bwF,3)
					obj.InputBuffer = bwF(:,:,m:-1:1);
				else
					obj.InputBuffer = repmat(bwF(:,:,1), 1, 1, m);
				end				
			end
	...
	methods (Access = protected)
		...
		function bwF = temporalRecurrenceFilter(obj, bwF, numPastFrames)
			...
			if nargin < 3
				numPastFrames = obj.pFilterNumFrames;
			end
		...
		function bwBufferedFrames = cycleInputBuffer(obj, bwF, numPastFrames)
			bwBufferedFrames = cat(3, obj.InputBuffer, bwF);
			...
			if isempty(obj.InputBuffer)
				obj.InputBuffer = bwBufferedFrames(:,:,nextBufIdx);
			else
				obj.InputBuffer(:,:,1:numPastFrames) = bwBufferedFrames(:,:,nextBufIdx); % end-nPastFrames+1:end
			end
		end
	end
	...
	methods (Hidden)
		function varargout = tuneInteractive(obj)
			...
			kstep = 2;
			obj.TuningStep(kstep).ParameterName = 'FilterNumFrames';
			x = obj.FilterNumFrames;
			if isempty(x)
				x = 3;
			end
		...
		function bwF = testTemporalFilter(obj, bwF)
			numPastFrames = obj.FilterNumFrames;
