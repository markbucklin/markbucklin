classdef  (CaseInsensitiveProperties = true) RigidMotionCorrector <  scicadelic.SciCaDelicSystem
	% RigidMotionCorrector - uses phase-correlation to find and correct the rigid displacement of frames over time.
	...
	properties (Nontunable, Logical)
		...
		AlwaysAlignToMovingAverage = false		
	...
	properties %(Nontunable)
		SkipPreviousAlignmentCorrelationThreshold = .9
		SkipMeanAlignmentCorrelationThreshold = .9
		FrameCorrWeightMask
	end
	...
	% CONSTRUCTOR
	methods
		function obj = RigidMotionCorrector(varargin)
	...
	methods (Access = protected)
		function setupImpl(obj,data,~)
			...
			% (NEW)
			if isempty(obj.FrameCorrWeightMask)
				obj.FrameCorrWeightMask = onGpu(obj, zeros(numRows, numCols, 'single'));
			end
	...
	methods (Access = protected)
		function [data, Uxy] = alignFrames(obj, data)
			...
			% ASSIGN ZERO-SHIFT TO FIRST FRAME & RETURN (may be unnecessary as taken care of above)
