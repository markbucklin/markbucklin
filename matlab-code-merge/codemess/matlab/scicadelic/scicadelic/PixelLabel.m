classdef (CaseInsensitiveProperties = true) PixelLabel < hgsetget
	...
	properties %(Constant)
		...
		DetectionStartDelay = 128				% Delay before measuring/accumulating input region properties
		RegistrationStartDelay = 256		% Delay before declaring/assigning tracked regions
		MinRegionArea = 12
	...
	methods
		...
		function initialize(obj, Q)
			...
			slut2 = false(512,1,'like',logical(Q)) | ignition.internal.lutmajority;
			obj.SpatialContinuityLut = {slut1, slut2};
			
			% CONSTRUCT EDGE LUT (INNER & OUTER COMBINED)
			elut1 = false(512,1,'like',logical(Q)) | ignition.internal.lutper4;
			elut2 = false(512,1,'like',logical(Q)) | ignition.internal.lutfatten;
			...
			if any(R(:))
				...
				if ~isempty(Sdet.Area)
					...
					if (Nk > obj.RegistrationStartDelay) && (numSeeds >= 1)
						...
						updateRegionRegistry(obj, Sdet, statMap, bwSeed);
	...
	methods
		function updateDetectedRegionEdgeMap(obj, R, Q)
			...
			Rd = Reio | R; % applySpatialLut(R,ignition.internal.lutdilate)
		...
		function S = getDetectedRegionProps(obj, R, kIdx1)
			...
			if isa(labelMat2d, 'gpuArray')
				labelMat2d = ignition.internal.gpu.intlut(uint16(labelMat2d), validAreaLabelLut); % variable type change
			else
