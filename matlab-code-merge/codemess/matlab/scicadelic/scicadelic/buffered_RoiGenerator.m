classdef (CaseInsensitiveProperties = true) RoiGenerator < scicadelic.SciCaDelicSystem
	...
	properties (Access = public)
		...
		TemporalFilterSpan = 3;						% Number of prior frames to compare to current frame in foreground operation, any matches propagate
	...
	properties (SetAccess = protected, Hidden)
		...
		FilledBuffer							% Buffer holding frames ready for segmentation
		UnfilledBuffer						% Circular buffer for holding frames passed to step -> transferred to FilledBuffer when full
		ForegroundBuffer					% Logical array representing unfiltered foreground pixels from all frames in Filled Buffer from pTemporalFilterSpan to end
	end
	...
	methods (Access = protected)
		function setupImpl(obj, data)
			...
			if inputNumFrames == 1
				obj.FilledBuffer = repmat(data,1,1,bufferSize);
			else
				obj.FilledBuffer = repmat(data,1,1,ceil(bufferSize/inputNumFrames));
			end
			obj.NFrames = 0;
			setPrivateProps(obj)
			bwFg = findPixelForeground(obj);
			tSpan = obj.pTemporalFilterSpan;
			obj.ForegroundBuffer = bwFg(:,:,tSpan:-1:1);
			% 			obj.ForegroundBuffer = cat(3, bwFg(:,:,1:tSpan),  bwFg);
			obj.IsTuned = false;
		end
		function stepImpl(obj, data)
			...
			obj.UnfilledBuffer(:,:,bufIdx(fitsInBuffer)) = data(:,:,fitsInBuffer);
			
			% RUN ROI-GENERATION IF BUFFER IS FULL
			if bufIdx(end) >= bufferSize
				obj.FilledBuffer = obj.UnfilledBuffer;
				processFilledBuffer(obj);
			end
			
			% PUT ANY REMAINING INPUT INTO NEXT BUFFER
			if any(~fitsInBuffer)
				nOver = nnz(~fitsInBuffer);
				obj.UnfilledBuffer(:,:,1:nOver) = data(~fitsInBuffer);
			end
	...
	methods (Access = protected)
		function processFilledBuffer(obj)
			...
			% APPLY BINARY TEMPORAL FILTER TO FOREGROUND
			bwFg = temporalFilterForeground(obj, bwFg);
			...
			findContiguousRegions(obj, bwFg);
		...
		function bwFg = temporalFilterForeground(obj, bwFg, tSpan)
			
			% RETRIEVE FROM BUFFER & APPLY TEMPORAL FILTER IF SPECIFIED
			if nargin < 3
				tSpan = obj.pTemporalFilterSpan;
			end
			if isempty(tSpan) || tSpan < 1				
				return
			else
				...
				obj.ForegroundBuffer(:,:,1:tSpan) = bwBufferedFg(:,:,end-tSpan+1:end);
				
				% INDEXED EXPANSION TO PAST FRAMES IN LOGICAL FILTERING OP
				idx = bsxfun(@plus, [0:numFrames-1]', 1:tSpan);
				
				% ANY PIXEL MATCHES BETWEEN CURRENT FRAME & PREVIOUS FRAMES ARE USED IN FOREGROUND
				% 				try
				bwFg = squeeze(any( bsxfun(@and,...
					bwBufferedFg(:,:,idx(:,end)+1),... %tSpan+1:end
					reshape(bwBufferedFg(:,:,idx(:)), nRows, nCols, numFrames, tSpan)), 4));
					% 				catch
		...
		function varargout = findContiguousRegions(obj, bwFg)
			% LOCAL VARIABLES
			...
			underMax = cellfun('length',cc.PixelIdxList) <= maxArea*bufSize;
	...
	methods (Hidden)
		function tuneInteractive(obj)
			...
			obj.TuningStep(2).ParameterName = 'TemporalFilterSpan';
			x = obj.TemporalFilterSpan;
			if isempty(x)
				x = 3;
			end
			...
			obj.TuningStep(2).Function = @testTemporalFilter;			
		...
		function F = testTemporalFilter(obj, F, tSpan)
			...
			% switches the current TEMPORALFILTERSPAN parameter value (determining the length of the
			...
			persistent lastTspan
			curIdx = obj.TuningImageIdx;
			numPreviousSteps = find(~strcmpi('TemporalFilterSpan', {obj.TuningStep.ParameterName}), 1, 'last');
			...
			if (curIdx ~= lastImageIdx) ...
				...
				% GRAB TSPAN FRAMES IMMEDIATELY PRECEDING THE CURRENT FRAME AND PERFORM ALL OPERATIONS
				if obj.UseGpu && ~isa(obj.TuningImageDataSet, 'gpuArray')
					fBuf = gpuArray( obj.TuningImageDataSet(:,:, max(1, curIdx-tSpan : curIdx-1)));
				else
					fBuf = obj.TuningImageDataSet(:,:, max(1, curIdx-tSpan : curIdx-1));
				end
				...
				% PUT TSPAN FRAMES IN FOREGROUND BUFFER
			...
			F = temporalFilterForeground(obj, F, tSpan);
...
% 					obj.TuningStep(1).ParameterName = 'TemporalFilterSpan';
% 					x = obj.TemporalFilterSpan;
% 					if isempty(x)
% 						x = 3;
% 					end
...
% 					obj.TuningFunction = @(F,tspan) any(bsxfun(@and, F(:,:,end-tspan:end-1), F(:,:,end)),3);					
% 					obj.TuningStep = 2;
% 					% 				case 3 % STEP 3: SPATIAL FILTER WITH MORPHOLOGICAL OPERATIONS
% 					% 					obj.TemporalFilterSpan = obj.TuningStep(1).ParameterDomain(obj.TuningStep(1).ParameterIdx);
% 					% 					setPrivateProps(obj)
% 					% 					obj.TuningImageDataSet = temporalFilterForeground(obj, obj.TuningImageDataSet);
