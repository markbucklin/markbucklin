...
for k=1:numel(vonames)
	imin(k) = prctile(double(vidOutput.(vonames{k})(1).cdata(:)), lowpct); 
	imax(k) = prctile(double(vidOutput.(vonames{k})(1).cdata(:)), highpct);
% 	imin(k) = double(min(vidOutput.(vonames{k})(1).cdata(2:end)));
% 	imax(k) = double(max(vidOutput.(vonames{k})(1).cdata(2:end)));
	nFrames = double(min(numel(vidOutput.(vonames{k})), nFrames));
end
wid = 1/numel(vonames);
